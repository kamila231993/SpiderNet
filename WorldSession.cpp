...
// Undeva la finalul WorldSession.cpp, înainte de orice #endif
void WorldSession::SendAuthResponse(uint8 code, bool shortForm, uint32 queuePos, uint32 waitTime)
{
    // codul tău aici
}
    WorldPacket data(SMSG_AUTH_RESPONSE, 1 + (shortForm ? 0 : 4));
    data << uint8(code);
    if (!shortForm)
