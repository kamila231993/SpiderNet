...
#include "fmt/format.h"  // dacă nu e deja inclus
bool IsDonatorAccount(uint32 accountId)
{
    std::string query = fmt::format("SELECT 1 FROM account_donator WHERE id = {}", accountId);
...
void WorldSessionMgr::AddQueuedPlayer(WorldSession* session)
{
    ...
    // Calculăm poziția în coadă și timpul estimativ
    int32 queuePos = GetQueuePos(session);
    uint32 waitTime = EstimateQueueWaitTime(session); // funcția ta deja există

    // Trimitem pachetul către client
    session->SendAuthResponse(AUTH_WAIT_QUEUE, false, queuePos, waitTime);
}
...
void WorldSessionMgr::AddSession_(WorldSession* session)
{
    ...
    // verificare GM sau donator
if (session->IsGMAccount() || IsDonatorAccount(session->GetAccountId()))
{
    if (session->IsGMAccount())
        LOG_INFO("server", "Account {} este GM, sare peste queue", session->GetAccountId());
    else
        LOG_INFO("server", "Account {} este donator, sare peste queue", session->GetAccountId());

    // inițializează sesiunea direct
    session->InitializeSession();
    UpdateMaxSessionCounters();
    return;
}

// dacă serverul e plin și NU e GM/donator → intră în queue
if (GetPlayerAmountLimit() && GetActiveSessionCount() >= GetPlayerAmountLimit())
{
    AddQueuedPlayer(session);
    LOG_INFO("server", "Account {} a intrat in queue", session->GetAccountId());
    UpdateMaxSessionCounters();
    return;
}

// altfel, server liber și nu e GM/donator → inițializează normal
session->InitializeSession();
UpdateMaxSessionCounters();
}
...
uint32 WorldSessionMgr::EstimateQueueWaitTime(WorldSession* session) const
{
    uint32 position = 1;

    // găsește poziția în coadă
    for (auto const& s : _queuedPlayer)
    {
        if (s == session)
            break;
        ++position;
    ...
    // Presupunem un timp mediu de procesare per jucător (poți modifica după nevoie)
    uint32 averageTimePerPlayer = 5; // secunde
    return position * averageTimePerPlayer;
}